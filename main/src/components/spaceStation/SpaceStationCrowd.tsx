...
import {
  buildSpaceStationCrowd,
  buildStallTraders,
  figureParts,
  poseAt,
  swungOffset,
  type CrowdAgent
} from '../../game/spaceStation/spaceStationCrowd.ts';
...
import { spaceStationVendorSites } from '../../game/spaceStation/spaceStationVendors.ts';
import { currentVisibleCells } from './SpaceStationInterior.tsx';
...
export function SpaceStationCrowd({ descriptor }: { descriptor: SpaceStationDescriptor }) {
  const bodyRef = useRef<THREE.InstancedMesh>(null);
  const visorRef = useRef<THREE.InstancedMesh>(null);

  const { agents, bodyCapacity, visorCapacity, partsByHeight } = useMemo(() => {
    const shoppers = buildSpaceStationCrowd(descriptor.graph, descriptor.seed);
    ...
    const traders = concourse
      ? buildStallTraders(concourse.id, spaceStationVendorSites(descriptor), descriptor.seed, shoppers.length)
      : [];
...
export function crowdSize(descriptor: SpaceStationDescriptor): number {
  const concourse = descriptor.graph.cells.find(cell => cell.kind === 'concourse');
  return (
    buildSpaceStationCrowd(descriptor.graph, descriptor.seed).length +
    (concourse ? spaceStationVendorSites(descriptor).length : 0)
  );
}
