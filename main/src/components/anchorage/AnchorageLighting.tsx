...
import type {
  ...
  AnchorageDescriptor,
  CellId
} from '../../game/anchorage/anchorageTypes.ts';

/**
 * Lighting for the anchorage interior. Two systems working together.
 *
 ...
 */
const DISTRICT_FILL: Record<AnchorageCellKind, DistrictFill> = {
...
export interface AnchorageLightingProps {
...
export function AnchorageLighting({ descriptor, occupiedCellId }: AnchorageLightingProps) {
  const lamps = useMemo(
    () =>
      buildAnchorageDressing(descriptor.graph, descriptor.seed)
  ...
  const kindByCell = useMemo(() => {
    const map = new Map<CellId, AnchorageCellKind>();
