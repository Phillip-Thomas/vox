...
import { buildAnchorageArchitecture } from '../../game/anchorage/anchorageArchitecture.ts';
...
import {
  ...
  SURFACE_STYLE,
  type AnchorageBox
} from '../../game/anchorage/anchorageShell.ts';
import type { AnchorageDescriptor, CellId } from '../../game/anchorage/anchorageTypes.ts';
import { visibleCells } from '../../game/anchorage/portalGraph.ts';
import { createStationMaterial, createStationTrimMaterial } from './stationMaterial.ts';

/**
 * Anchorage interior: shell architecture plus dressing, culled by the portal graph.
 *
...
export interface AnchorageInteriorDiagnostics {
  worldId: string;
  cellCount: number;
...
let latestDiagnostics: AnchorageInteriorDiagnostics | null = null;
let latestVisibleCells: ReadonlySet<CellId> = new Set();

/** Read by capture harnesses and the sandbox HUD. Never drives rendering. */
export function anchorageInteriorDiagnostics(): AnchorageInteriorDiagnostics | null {
...
export interface AnchorageInteriorProps {
  descriptor: AnchorageDescriptor;
  /** Cell the viewer currently stands in, from the locomotion state. */
  occupiedCellId: CellId | null;
}

export function AnchorageInterior({ descriptor, occupiedCellId }: AnchorageInteriorProps) {
  ...
  const { surfacesByCell, lampsByCell, surfaceCapacity, lampCapacity } = useMemo(() => {
    const all = [
      ...buildAnchorageShell(graph),
      ...buildAnchorageArchitecture(graph, descriptor.seed),
      ...buildAnchorageDressing(graph, descriptor.seed)
    ];
    const surfaces = new Map<string, AnchorageBox[]>();
    const lamps = new Map<string, AnchorageBox[]>();
    ...
    const total = (map: Map<string, AnchorageBox[]>) =>
  ...
  useFrame(({ camera, gl }) => {
    ...
    const pack = (
      ...
      source: Map<string, AnchorageBox[]>,
      palette: THREE.Color[],
