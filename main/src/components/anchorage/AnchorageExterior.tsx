...
import {
  buildAnchorageExterior,
  EXTERIOR_LIGHT_TONE,
  type ExteriorTier
} from '../../game/anchorage/anchorageExterior.ts';
...
export interface AnchorageExteriorProps {
...
export function AnchorageExterior({
  ...
  const body = useMemo(
    () => suppliedBody ?? anchorageBody(descriptor.address, descriptor.graph),
    [descriptor, suppliedBody]
  );

  const exterior = useMemo(
    () => buildAnchorageExterior(descriptor.graph, descriptor.seed),
    [descriptor]
  );
...
export interface AnchorageExteriorDiagnostics {
  range: number;
...
let latestDiagnostics: AnchorageExteriorDiagnostics | null = null;

/** Read by the sandbox HUD and capture harnesses. Never drives rendering. */
export function anchorageExteriorDiagnostics(): AnchorageExteriorDiagnostics | null {
