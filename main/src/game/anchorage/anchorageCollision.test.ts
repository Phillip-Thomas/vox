...
import { buildAnchorageDressing } from './anchorageDressing.ts';
import { createWalkState, PLAYER_BODY, stepWalk, type WalkState } from './anchorageLocomotion.ts';
import type { AnchorageBox } from './anchorageShell.ts';
import { anchorageVendorSites } from './anchorageVendors.ts';
...
describe('walking the real concourse', () => {
  ...
  const props = buildBlockerIndex(buildAnchorageDressing(graph, descriptor.seed));
  ...
  it('cannot walk through a vendor counter into the stall', () => {
    const site = anchorageVendorSites(descriptor).find(entry => entry.side === 1)!;
  ...
  it('leaves the trader standing clear of her own stall', () => {
    for (const site of anchorageVendorSites(descriptor)) {
