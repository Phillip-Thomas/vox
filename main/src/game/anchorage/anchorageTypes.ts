...
/**
 * An anchorage is a built deep-space destination: you fly to it, dock, and walk
 ...
 * anchorages carry their own parallel address space instead of widening that union.
 */
export interface AnchorageAddress {
  system: SystemCoordinate;
  /** Index within the system. Systems may eventually hold more than one. */
  index: number;
}

export interface AnchorageIdentity {
  worldId: string;
  seed: number;
  systemId: string;
  address: AnchorageAddress;
  anchorageIdentityVersion: number;
}

export type AnchorageCellKind =
 ...
 */
export interface AnchorageCell {
  id: CellId;
 ...
 */
export interface AnchoragePortal {
  id: string;
  a: CellId;
  b: CellId;
...
export interface AnchorageGraph {
  cells: AnchorageCell[];
  portals: AnchoragePortal[];
}

export interface AnchorageDescriptor extends AnchorageIdentity {
