...
import {
  ...
  EXTERIOR_LIGHT_TONE,
  type ExteriorBox
} from './anchorageExterior.ts';

const ADDRESS = { system: { x: -19, y: -17 }, index: 0 };
const descriptor = buildAnchorageDescriptor(ADDRESS);
const exterior = buildAnchorageExterior(descriptor.graph, descriptor.seed);
const body = anchorageBody(ADDRESS, descriptor.graph);
...
describe('exterior massing', () => {
  ...
  it('is deterministic from the seed', () => {
    const again = buildAnchorageExterior(descriptor.graph, descriptor.seed);
