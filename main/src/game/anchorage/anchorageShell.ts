...
export function buildAnchorageShell(graph: AnchorageGraph): AnchorageBox[] {
...
export function buildCellShell(cell: AnchorageCell, portals: AnchoragePortal[]): AnchorageBox[] {
  const boxes: AnchorageBox[] = [];
 ...
 * real polygon split; the layout never produces that, and `validateAnchorageShell`
...
function buildWall(
  ...
  ].filter((box): box is AnchorageBox => box !== null);
}

/** A raised lip around an opening so a doorway reads as built rather than cut. */
function buildFrame(opening: AnchoragePortal, normalAxis: 0 | 2, plane: number): AnchorageBox[] {
  const [u, v] = normalAxis === 0 ? ([2, 1] as const) : ([0, 1] as const);
...
export function touchesCell(portal: AnchoragePortal, cell: AnchorageCell): boolean {
  return portal.a === cell.id || portal.b === cell.id;
}

function portalLiesOnFace(portal: AnchoragePortal, axis: 0 | 2, plane: number): boolean {
...
function mid(cell: AnchorageCell, axis: 0 | 1 | 2): number {
  return (cell.min[axis] + cell.max[axis]) / 2;
}

/** Reports layout shapes this shell builder cannot represent honestly. */
export function validateAnchorageShell(graph: AnchorageGraph): string[] {
