...
import { anchorageSpawnPoint, buildAnchorageDescriptor } from './anchorageDescriptor.ts';
import {
  createWalkState,
  eyePosition,
  PLAYER_EYE_HEIGHT,
...
function walk(state: WalkState, dx: number, dz: number, seconds: number, world: AnchorageGraph = graph): WalkState {
...
describe('walk state', () => {
  it('spawns standing in the apron', () => {
    const state = createWalkState(graph, anchorageSpawnPoint(descriptor));
  ...
  it('falls to the floor when spawned in the air', () => {
    const spawn = anchorageSpawnPoint(descriptor);
...
describe('step determinism', () => {
  it('produces identical results for identical inputs', () => {
    const a = walk(createWalkState(graph, anchorageSpawnPoint(descriptor)), 7, 3, 5);
    const b = walk(createWalkState(graph, anchorageSpawnPoint(descriptor)), 7, 3, 5);
    expect(a).toEqual(b);
  });
