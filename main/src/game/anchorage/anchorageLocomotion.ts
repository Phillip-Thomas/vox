...
import {
  ...
  type BlockerIndex,
  type BodyMetrics
} from './anchorageCollision.ts';
import { WALL_THICKNESS } from './anchorageShell.ts';
import type { AnchorageCell, AnchorageGraph, AnchoragePortal, CellId } from './anchorageTypes.ts';

/**
 * Walking inside an anchorage.
 *
...
export function createWalkState(graph: AnchorageGraph, spawn: Vec3Tuple): WalkState {
 ...
 */
function inDoorwaySlab(portal: AnchoragePortal, point: Vec3Tuple): boolean {
  const normalAxis = portalNormalAxis(portal);
...
function portalNormalAxis(portal: AnchoragePortal): number {
  return portal.halfExtents.findIndex(extent => extent === 0);
}

function cellCentreAxis(cell: AnchorageCell, axis: number): number {
...
function withinCellFootprint(cell: AnchorageCell, point: Vec3Tuple, radius: number): boolean {
  return (
...
function clampAxis(cell: AnchorageCell, axis: 0 | 2, value: number): number {
  return Math.min(Math.max(value, cell.min[axis] + PLAYER_RADIUS), cell.max[axis] - PLAYER_RADIUS);
}

function cellById(graph: AnchorageGraph, id: CellId | null): AnchorageCell | null {
  if (!id) return null;
  return graph.cells.find(cell => cell.id === id) ?? null;
}

function enterableCellAt(graph: AnchorageGraph, point: Vec3Tuple): AnchorageCell | null {
  return (
    graph.cells.find(
      cell =>
        cell.enterable &&
