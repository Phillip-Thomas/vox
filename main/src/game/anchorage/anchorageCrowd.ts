...
import type { AnchorageCell, AnchorageGraph, CellId } from './anchorageTypes.ts';
...
export function buildAnchorageCrowd(graph: AnchorageGraph, seed: number): CrowdAgent[] {
...
function populationFor(kind: AnchorageCell['kind']): number {
  switch (kind) {
    case 'concourse':
...
function createAgent(id: number, cell: AnchorageCell, cellSeed: number, index: number): CrowdAgent {
 ...
 * layer, so the anchorage crowd and W-7744 are recognisably the same species of
