...
import {
  anchorageBoundRadius,
  buildAnchorageGraph,
  reachableCells,
  validateAnchorageGraph
} from './anchorageLayout.ts';
import { anchorageSeedForAddress } from './anchorageAddress.ts';
import { cellContainingPoint, visibleCells } from './portalGraph.ts';
import type { AnchorageGraph } from './anchorageTypes.ts';

const SEEDS = Array.from({ length: 64 }, (_, index) =>
  anchorageSeedForAddress({ system: { x: index - 32, y: index * 7 - 11 }, index: 0 })
);

describe('anchorage layout generation', () => {
  it('produces a valid graph for every seed it can be handed', () => {
    for (const seed of SEEDS) {
      const problems = validateAnchorageGraph(buildAnchorageGraph(seed));
      expect(problems, `seed ${seed}`).toEqual([]);
    }
  });

  it('is deterministic for a given seed and varies across seeds', () => {
    const first = buildAnchorageGraph(SEEDS[0]);
    expect(buildAnchorageGraph(SEEDS[0])).toEqual(first);

    const footprints = new Set(SEEDS.map(seed => JSON.stringify(buildAnchorageGraph(seed).cells)));
    expect(footprints.size).toBeGreaterThan(1);
  });

  it('keeps every enterable cell walkable and the blank sealed off', () => {
    const graph = buildAnchorageGraph(SEEDS[0]);
  ...
  it('leaves no gap between consecutive volumes along the spine', () => {
    const graph = buildAnchorageGraph(SEEDS[3]);
  ...
  it('reports a bound radius enclosing the whole structure', () => {
    const graph = buildAnchorageGraph(SEEDS[1]);
    const radius = anchorageBoundRadius(graph);
  ...
  it('places the dock spawn inside the apron', () => {
    const graph = buildAnchorageGraph(SEEDS[2]);
...
describe('generator assertions actually bite', () => {
  const mutate = (mutation: (graph: AnchorageGraph) => void): string[] => {
    const graph = buildAnchorageGraph(SEEDS[0]);
    mutation(graph);
    return validateAnchorageGraph(graph);
  };
