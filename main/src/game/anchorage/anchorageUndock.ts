...
import { parseAnchorageWorldId } from './anchorageAddress.ts';
...
export function parseUndockFlag(raw: string | null): AnchorageAddress | null {
  if (!raw) return null;
  ...
  const asWorldId = parseAnchorageWorldId(raw);
...
export function requestedUndockAddress(search: string = typeof window === 'undefined' ? '' : window.location.search): AnchorageAddress | null {
 ...
 */
export function undockedShipPose(address: AnchorageAddress): SystemShipPose {
