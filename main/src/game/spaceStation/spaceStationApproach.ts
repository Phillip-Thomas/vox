...
import { systemToStationLocal, type SpaceStationBody } from './spaceStationBody.ts';

/**
 * Flying to an spaceStation, and being allowed to dock.
 *
...
export function evaluateApproach(body: SpaceStationBody, ship: ApproachInput): ApproachReadout {
  ...
  if (distance > SCAN_RANGE) {
    return {
      ...
      canDock: false,
      blocker: 'range',
      advisory: 'no spaceStation on instruments'
    };
  }
  ...
  if (distance > CORRIDOR_RANGE) {
    return {
      ...
      canDock: false,
      blocker: 'range',
      advisory: `spaceStation ${Math.round(distance)} out · hold for approach`
    };
  }
 ...
 */
export function offAxisAngle(body: SpaceStationBody, position: Vec3Tuple): number {
 ...
 */
export function approachHold(body: SpaceStationBody, distanceFromBerth: number): Vec3Tuple {
  return [
 ...
 */
export function arrivalStandoff(body: SpaceStationBody, distanceFromCentre: number): Vec3Tuple {
 ...
 */
export function keyLightDirection(body: SpaceStationBody): Vec3Tuple {
  const lateral = perpendicularTo(body.approachAxis);
  const up = cross(body.approachAxis, lateral);
  return normalize([
