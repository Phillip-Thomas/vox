...
import type { SpaceStationCell, SpaceStationGraph } from './spaceStationTypes.ts';
...
export function buildSpaceStationDressing(graph: SpaceStationGraph, seed: number): SpaceStationBox[] {
...
function dressCell(cell: SpaceStationCell, seed: number): SpaceStationBox[] {
  switch (cell.kind) {
    case 'apron':
...
function dressApron(cell: SpaceStationCell, seed: number): SpaceStationBox[] {
  const boxes: SpaceStationBox[] = [];
...
function dressCounter(cell: SpaceStationCell, seed: number): SpaceStationBox[] {
  const boxes: SpaceStationBox[] = [];
  const floorY = cell.min[1];
  const length = cell.max[0] - cell.min[0];
...
export function concourseStallSites(cell: SpaceStationCell, seed: number): StallSite[] {
...
function dressConcourse(cell: SpaceStationCell, seed: number): SpaceStationBox[] {
  const boxes: SpaceStationBox[] = [];
  const floorY = cell.min[1];
  const length = cell.max[0] - cell.min[0];
 ...
 */
function dressFloor(cell: SpaceStationCell, seed: number): SpaceStationBox[] {
  const boxes: SpaceStationBox[] = [];
  const floorY = cell.min[1];
...
function dressShelves(cell: SpaceStationCell, seed: number): SpaceStationBox[] {
  const boxes: SpaceStationBox[] = [];
  const floorY = cell.min[1];
  const spanX = cell.max[0] - cell.min[0];
  const spanZ = cell.max[2] - cell.min[2];
...
function ceilingStrips(cell: SpaceStationCell, seed: number, count: number): SpaceStationBox[] {
  const boxes: SpaceStationBox[] = [];
  const spanX = cell.max[0] - cell.min[0];
...
export function boxCenterInside(box: SpaceStationBox, cell: SpaceStationCell): boolean {
