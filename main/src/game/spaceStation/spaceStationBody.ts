...
import { buildSpaceStationGraph } from './spaceStationLayout.ts';
import type { SpaceStationAddress, SpaceStationGraph } from './spaceStationTypes.ts';

/**
 * The spaceStation as a body in system space.
 *
...
/** SpaceStations sit beyond the planet band so approach reads as deep space. */
const MIN_SPACE_STATION_RADIUS = MAX_COMPANION_DISTANCE_FROM_PRIMARY + 1_400;
const SPACE_STATION_RADIUS_SPREAD = 2_400;
...
/** Salts frozen at v1: changing one relocates every existing spaceStation. */
const SALT = {
  population: 401,
  azimuth: 307,
  elevation: 311,
 ...
 */
export interface SpaceStationFrame {
...
export interface SpaceStationBody {
  worldId: string;
  seed: number;
...
/**
 * How many spaceStations a system has.
 *
 ...
 */
export function systemSpaceStationCount(coordinate: SystemCoordinate, systemSeed: number): 0 | 1 | 2 {
  void coordinate;
...
/**
 * Every spaceStation in a system, in index order. Empty for most systems.
 *
 ...
 */
export function systemSpaceStations(
  coordinate: SystemCoordinate,
  systemSeed: number,
  minimum = 0
): SpaceStationBody[] {
  const count = Math.max(minimum, systemSpaceStationCount(coordinate, systemSeed));
  const bodies: SpaceStationBody[] = [];
  for (let index = 0; index < count; index++) {
    bodies.push(spaceStationBody({ system: coordinate, index }));
  }
  return bodies;
}

export function spaceStationBody(address: SpaceStationAddress, graph?: SpaceStationGraph): SpaceStationBody {
  const identity = createSpaceStationIdentity(address);
  const seed = identity.seed;
  const resolved = graph ?? buildSpaceStationGraph(seed);
  const frame = spaceStationFrame(resolved);

  const systemPosition = systemPositionForSpaceStation(seed);
...
function dockCell(graph: SpaceStationGraph) {
  return (
...
export function spaceStationFrame(graph: SpaceStationGraph): SpaceStationFrame {
...
export function toStationLocal(frame: SpaceStationFrame, point: Vec3Tuple): Vec3Tuple {
  return [point[0] - frame.origin[0], point[1] - frame.origin[1], point[2] - frame.origin[2]];
}

/** Station-local coordinate to system space. */
export function stationLocalToSystem(body: SpaceStationBody, local: Vec3Tuple): Vec3Tuple {
  return addTuple(body.systemPosition, rotate(body.quaternion, local));
}

/** System coordinate to station-local. Inverse of the above. */
export function systemToStationLocal(body: SpaceStationBody, point: Vec3Tuple): Vec3Tuple {
...
function systemPositionForSpaceStation(seed: number): Vec3Tuple {
  const azimuth = seededUnit(seed, SALT.azimuth) * Math.PI * 2;
  const elevation = (seededUnit(seed, SALT.elevation) - 0.5) * 0.5;
  const radius = MIN_SPACE_STATION_RADIUS + seededUnit(seed, SALT.radius) * SPACE_STATION_RADIUS_SPREAD;
