...
import { spaceStationFrame, toStationLocal, type SpaceStationFrame } from './spaceStationBody.ts';
import type { SpaceStationCell, SpaceStationCellKind, SpaceStationGraph } from './spaceStationTypes.ts';
 ...
 * orienting it in system space is `spaceStationBody`'s job.
 */
...
export interface SpaceStationExterior {
  boxes: ExteriorBox[];
...
const WINDOW_TONE: Record<SpaceStationCellKind, number | null> = {
...
export function buildSpaceStationExterior(graph: SpaceStationGraph, seed: number): SpaceStationExterior {
...
function bulkheadCollars(cells: SpaceStationCell[], frame: SpaceStationFrame, boxes: ExteriorBox[]): void {
...
function navLights(cells: SpaceStationCell[], frame: SpaceStationFrame, lights: ExteriorLight[]): void {
  for (const cell of cells) {
...
function hullToneFor(kind: SpaceStationCellKind): number {
  switch (kind) {
    case 'apron':
      return 1;
    case 'concourse':
