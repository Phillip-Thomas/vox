...
import { COMMODITY_IDS, type CommodityId, type CommodityTier } from './spaceStationCommodities.ts';
import { concourseStallSites, type StallSite } from './spaceStationDressing.ts';
import type { Market, MarketShock, MarketStock } from './spaceStationMarket.ts';
import type { SpaceStationDescriptor, CellId } from './spaceStationTypes.ts';
...
/**
 * The pitches this spaceStation's traders occupy.
 *
 * Separate from `buildSpaceStationVendors` because the vendor record carries a live
 ...
 */
export function spaceStationVendorSites(descriptor: SpaceStationDescriptor): StallSite[] {
...
export function buildSpaceStationVendors(descriptor: SpaceStationDescriptor): Vendor[] {
  const concourse = descriptor.graph.cells.find(cell => cell.kind === 'concourse');
  if (!concourse) return [];

  return spaceStationVendorSites(descriptor).map((site, index) => {
