...
import { buildSpaceStationDescriptor, spaceStationDockRoute, spaceStationSpawnPoint } from './spaceStationDescriptor.ts';
import {
  advanceDock,
  beginDock,
  beginUndock,
...
describe('dock route', () => {
  ...
  const route = spaceStationDockRoute(descriptor);
  ...
  it('sets you down exactly where a plain spawn would put you', () => {
    expect(spaceStationSpawnPoint(descriptor)).toEqual(route.deck);
  });
