...
import {
  ...
  evaluateApproach,
  offAxisAngle
} from './spaceStationApproach.ts';
import {
  spaceStationBody,
  spaceStationFrame,
  stationLocalToSystem,
  systemSpaceStationCount,
  systemSpaceStations,
  systemToStationLocal
} from './spaceStationBody.ts';
import { spaceStationDockRoute, buildSpaceStationDescriptor } from './spaceStationDescriptor.ts';
...
describe('spaceStation as a system body', () => {
  ...
  it('is deterministic from its address', () => {
    expect(spaceStationBody(ADDRESS)).toEqual(body);
  });

  it('agrees with the descriptor on where the station is', () => {
    expect(buildSpaceStationDescriptor(ADDRESS).systemPosition).toEqual(body.systemPosition);
  });

  it('populates most systems with nothing at all', () => {
    let withStation = 0;
    for (let seed = 1; seed <= 400; seed++) {
      if (systemSpaceStationCount({ x: 0, y: 0 }, seed * 2_654_435_761 >>> 0) > 0) withStation++;
    }
  ...
  it('gives every spaceStation in a system a distinct id and place', () => {
    // Find a seed that actually produces two, rather than asserting on one that may not.
    let pair: ReturnType<typeof systemSpaceStations> = [];
    for (let seed = 1; seed <= 4_000 && pair.length < 2; seed++) {
      pair = systemSpaceStations({ x: 3, y: 4 }, (seed * 2_246_822_519) >>> 0);
    }
...
describe('station frame', () => {
  const frame = spaceStationFrame(buildSpaceStationGraph(body.seed));
...
describe('the berth lines up with the airlock', () => {
  it('sits off the dock end of the station, on the dock mouth axis', () => {
    ...
    const graph = buildSpaceStationGraph(body.seed);
    const apron = graph.cells.find(cell => cell.kind === 'apron')!;
    const frame = spaceStationFrame(graph);
  ...
  it('is on the same end of the spine as the interior dock route', () => {
    ...
    const frame = spaceStationFrame(descriptor.graph);
    const lock = spaceStationDockRoute(descriptor).lock;
    const deck = spaceStationDockRoute(descriptor).deck;
...
describe('approach corridor', () => {
  ...
  it('picks the station up on instruments inside scan range', () => {
    ...
    expect(readout.advisory).toMatch(/spaceStation/);
  });
