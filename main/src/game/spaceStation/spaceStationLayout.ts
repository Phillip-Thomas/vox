...
import type {
  ...
  SpaceStationGraph,
  SpaceStationPortal
} from './spaceStationTypes.ts';

/**
 * The spaceStation layout generator.
 *
...
export const SPACE_STATION_LAYOUT_VERSION = 1;

/** Salts are arbitrary but frozen — changing one re-rolls every existing spaceStation. */
const SALT = {
  apronLength: 211,
  apronHeight: 223,
  counterLength: 227,
...
export function buildSpaceStationGraph(seed: number): SpaceStationGraph {
  ...
  const portals: SpaceStationPortal[] = [
...
export function spaceStationBoundRadius(graph: SpaceStationGraph): number {
  let maxDistance = 0;
 ...
 */
export function validateSpaceStationGraph(graph: SpaceStationGraph): string[] {
...
export function reachableCells(graph: SpaceStationGraph): Set<string> {
...
function cellCorners(cell: SpaceStationCell): Vec3Tuple[] {
