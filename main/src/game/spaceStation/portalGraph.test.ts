...
const cell = (id: string, minX: number, maxX: number): SpaceStationCell => ({
  id,
  kind: 'floor',
  min: [minX, 0, -6],
  max: [maxX, 4, 6],
...
function corridorGraph(overrides: Partial<SpaceStationPortal> = {}): SpaceStationGraph {
  return {
...
describe('visibility through portals', () => {
  ...
  it('clips an opening that straddles the near plane instead of mirroring it', () => {
    ...
    const straddling: SpaceStationPortal = {
      id: 'straddle',
      a: 'a',
      b: 'side',
      center: [10, 2, 6],
