 ...
 *
 * Loads the shipped app in deep space (?fly=1) rather than the anchorage sandbox,
 ...
 *
 * Usage: node tools/anchorage-system-probe.mjs --out <dir> [--url http://localhost:5180]
 */
...
const OUT = resolve(argValue('out', 'captures/anchorage/system'));
...
const contacts = await page.evaluate(() => window.__anchorageContacts?.() ?? null);
...
for (let i = 0; i < 14; i++) {
  ...
  const now = await page.evaluate(() => window.__anchorageContacts?.() ?? null);
...
const closed = await page.evaluate(() => window.__anchorageContacts?.() ?? null);
...
const hud = await page.evaluate(() =>
  document.querySelector('[data-testid="anchorage-approach-hud"]')?.textContent ?? null);
