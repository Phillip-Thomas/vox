#!/usr/bin/env node
/**
 * Anchorage look-development capture harness.
 *
 ...
 *
 * Usage:
 *   node tools/anchorage-shots.mjs --out <dir> [--url http://localhost:4321] [--label v3]
 *
...
const OUT = resolve(argValue('out', 'captures/anchorage/latest'));
...
await page.goto(`${BASE}/?anchorage=1&hud=0&t=17.5&dock=0`, { waitUntil: 'networkidle', timeout: 90_000 });
await page.waitForTimeout(4_500);

for (const stop of STOPS) {
  const placed = await page.evaluate(
    ([cell, u, v]) => window.__anchorageTeleport?.(cell, u, v) ?? false,
  ...
  await page.evaluate(([yaw, pitch]) => {
    window.__anchorageLook?.(yaw, pitch);
  ...
  const state = await page.evaluate(() => window.__anchorageState?.() ?? null);
