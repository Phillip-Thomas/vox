 ...
 *
 * Usage:
 *   node tools/anchorage-arrival-probe.mjs --out <dir> [--url http://localhost:5180]
 */
...
const OUT = resolve(argValue('out', 'captures/anchorage/arrival'));
...
await page.goto(`${BASE}/?anchorage=1&hud=0`, { waitUntil: 'domcontentloaded', timeout: 90_000 });
await page.waitForSelector('[data-testid="anchorage-dock-overlay"]', { timeout: 60_000 });
...
const overlayGone = await page
  .waitForSelector('[data-testid="anchorage-dock-overlay"]', { state: 'detached', timeout: 120_000 })
  .then(() => true)
  .catch(() => false);
check('dock sequence completes and hands control back', overlayGone);

const arrived = await page.evaluate(() => window.__anchorageState?.() ?? null);
...
const before = await page.evaluate(() => {
  const id = window.__anchorageTeleportToVendor?.(2);
  return { id, state: window.__anchorageState?.() ?? null };
});
check('teleported to a vendor counter', Boolean(before.id), before.id ?? 'none');

await page.evaluate(() => window.__anchorageWalk?.(6, 0));
await page.waitForTimeout(3_000);
const after = await page.evaluate(() => {
  window.__anchorageWalk?.(0, 0);
  return window.__anchorageState?.() ?? null;
});
...
// ------------------------------------------------------------ staffed stalls
const staffing = await page.evaluate(() => window.__anchorageStaffing?.() ?? null);
check(
