...
async function captureHarness(page, name, url, viewport, minTrees) {
  ...
  proof.pass = !proof.frameworkOverlay &&
    ...
    proof.materialKeys.includes('tree-leaf-v4') &&
...
async function captureWorld(page, name, url, viewport, viewName) {
  ...
  proof.path = file;
  proof.pass = !proof.frameworkOverlay &&
    proof.meshes.some(mesh => mesh.key === 'tree-leaf-v4' && mesh.count > 0 && mesh.vertices > 0) &&
...
const result = {
  ...
  consoleIssues,
  failedResponses,
  relevantConsoleIssues: relevantIssues(consoleIssues, failedResponses).filter(issue => {
