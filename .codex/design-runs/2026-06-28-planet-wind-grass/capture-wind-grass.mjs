...
function grassProbeScript() {
  ...
  const u = mesh.material?.userData?.shader?.uniforms ?? {};
  const vec = value => value ? { x: value.x, y: value.y } : null;
  result.windUniforms = {
    dir: vec(u.uWindDir?.value),
    ...
    offset: vec(u.uWindOffset?.value)
  };
...
const desktopWide = await capture(
  ...
  async p => p.evaluate(() => {
    let grass = null;
    window.__game.scene.traverse(object => {
      if (grass || !object.isInstancedMesh) return;
      let key = '';
      try { key = object.material?.customProgramCacheKey?.() ?? ''; } catch { /* ignore */ }
      if (/grass-pbr/.test(key)) grass = {
        count: object.count,
        key,
...
const desktopWind = desktopClose.proof.windUniforms;
if (
  ...
  desktopClose.proof.grassKey !== 'grass-pbr-v5' ||
  !desktopWind ||
  desktopWind.gustStrength == null ||
  desktopWind.gustScale == null ||
  desktopWind.gustSpeed == null ||
  desktopWind.turbulence == null ||
  desktopWind.veer == null ||
