...
const RESOLVE = async () => {
  ...
  const want = {
    ...
    cockpitLayout: /shipCockpitLayout\.ts/
  };
  const found = {};
...
const COMPOSE = () => {
  ...
  if (hull && lights && !hull.instanceColor && lights.instanceColor) { const t = hull; hull = lights; lights = t; }
...
const OUT = path.join(OUT_DIR, `ch10-v7-${MODE}.json`);
...
if (MODE === 'cutline') {
  ...
  report.still = { id: 'still-station-resolved', file: 'evidence/capture/still-station-resolved.png',
    ...
    spineSpec: [28, 34], stationMask: masks?.station ?? null,
